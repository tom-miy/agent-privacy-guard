...
- JSONL audit logging。
- sample hooks、templates、adapters、demo inputs。
...
| 2. preview | `templates/agent-privacy-guard/policy.yaml` の `entities` と built-in detector で何が置換されるか | `AcmeBank -> [CLIENT#A]` のように表示される |
...
sample policy は `templates/agent-privacy-guard/policy.yaml` にあります。ここで「どの target に送るか」と「何をどの placeholder にするか」を設定します。
...
重要: 本物の顧客名、内部サービス名、DB 名などは、それ自体が機密情報になり得ます。`templates/agent-privacy-guard/policy.yaml` に直書きして git 管理しないでください。本番では `entity_files` で `.agent-privacy-guard/entities.local.yaml` のような gitignore 済み local file から読み込むか、SOPS / age / git-crypt などで暗号化して管理し、実行時に復号した file を読み込む運用を推奨します。空の local entity file は設定済みに見えてしまうため、この repository では実ファイルではなく example のみを置きます。
...
install 後の `.agent-privacy-guard/policy.yaml` と想定される agent config file を validate します。

```bash
agent-privacy-guard validate
```

この source repository 内で template を検証する場合は、明示的に path を指定します。
...
| File | Purpose |
|---|---|
| `templates/agent-privacy-guard/policy.yaml` | target policy、entity rule、outbound control。 |
| `templates/agent-privacy-guard/entities.local.example.yaml` | gitignore する local entity file の sample。 |
| `docs/policy-config.ja.md` | `templates/agent-privacy-guard/policy.yaml` の layout と field reference。 |
...
| `templates/agent-privacy-guard/mcp-trust.yaml` | MCP server ごとの trust metadata。通常の MCP 起動 config ではない。 |
| `docs/mcp-trust-config.ja.md` | `templates/agent-privacy-guard/mcp-trust.yaml` の layout と通常の MCP config との違い。 |
...
本番の entity rule に本物の顧客名や内部識別子を含める場合は、`templates/agent-privacy-guard/policy.yaml` ではなく gitignore 済みの `.agent-privacy-guard/entities.local.yaml` または暗号化管理された file を使ってください。
